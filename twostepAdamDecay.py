...
best_epoch = -1  # Initialize best_epoch to track the epoch with the lowest loss
best_weights = None  # Initialize best_weights to store the weights of the best epoch
...
for epoch in range(nep):
    ...
    # After computing accuracy and average loss for the epoch
    if average_loss < best_loss:
        best_loss = average_loss  # Update best_loss
        best_epoch = epoch  # Update best_epoch
        best_weights = W.copy()  # Save the current state of the weights
    elif epoch - best_epoch >= decay_steps:  # patience is the number of epochs to wait after the last improvement
        print("Early stopping as there's no improvement for {patience} consecutive epochs.")
        break  # Stop training if there hasn't been an improvement in 'patience' epochs
...
for epoch in range(nep):
    ...
    for i in range(0, N, btch):
        ...
        for j in range(x_batch.shape[0]):
            ...
            m_hat = m / (1 - beta1 ** K)
            v_hat = v / (1 - beta2 ** K)
            W_nocancer -= alpha * m_hat / (np.sqrt(v_hat) + epsilon)
...
m = np.zeros_like(W_cancerous)
v = np.zeros_like(W_cancerous)
...
for epoch in range(nep):
    ...
    for i in range(0, N, btch):
        ...
        for j in range(x_batch.shape[0]):
            ...
            m_hat = m / (1 - beta1 ** K)
            v_hat = v / (1 - beta2 ** K)
            W_cancerous -= alpha * m_hat / (np.sqrt(v_hat) + epsilon)
...
# Load test data
test_images = ['./dataverse_files/ISIC2018_Task3_Test_Images']  # Make sure the path is correct
...
# Plot for cancerous classes training
plt.plot(range(1, len(losses) + 1), losses, label='Cancerous Classes')

# Plot for non-cancerous classes training
plt.plot(range(1, len(losses_nocancer) + 1), losses_nocancer, label='Non-cancerous Classes')

# Plot for binary classification training
plt.plot(range(1, len(losses_cancerous) + 1), losses_cancerous, label='Binary Classification')
...
# Plot for cancerous classes accuracy
plt.plot(range(1, len(accuracies) + 1), accuracies, label='Cancerous Classes')

# Plot for non-cancerous classes accuracy
plt.plot(range(1, len(accuracies_nocancer) + 1), accuracies_nocancer, label='Non-cancerous Classes')

# Plot for binary classification accuracy
plt.plot(range(1, len(accuracies_cancerous) + 1), accuracies_cancerous, label='Binary Classification')
